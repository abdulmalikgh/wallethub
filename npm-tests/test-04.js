...
app.post('/save',(request,reply)=>{
    ...
    userInput.firstname = body.firstname.replace(/<[^>]*>/g, '').replace(/[^a-zA-Z0-9]/g,'').replace(/alert(1)/g,'').replace(/javascript/g,'');
    userInput.lastname = body.lastname.replace(/<[^>]*>/g, '').replace(/[^a-zA-Z0-9]/g,'').replace(/alert(1)/g,'').replace(/javascript/g,'');
...
app.listen(8080,"0.0.0.0").then((address)=>{
    ...
    let req = http.request('http://127.0.0.1:8080/save',options, res => {
        res.on('data', d => {
            console.log(d)
        })
    })
